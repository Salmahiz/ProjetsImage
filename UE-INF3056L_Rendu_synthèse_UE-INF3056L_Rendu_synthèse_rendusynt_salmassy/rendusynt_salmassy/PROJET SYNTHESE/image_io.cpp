...
// applique correction gamma a une couleur
Color modgamma(const Color& color, const float g )
{
    return Color(std::pow(color.r, g), std::pow(color.g, g), std::pow(color.b, g), color.a);
}

// gamma sur image entiere
Image modgamma(const Image& image, const float g)
{
    Image tmp(image.width(), image.height());
    
    float invg = 1 / g;
    for(unsigned i= 0; i < image.size(); i++)
        tmp(i)= modgamma(image(i), invg);
    
    return tmp;
}

// inverse du gamma
Image inverse_modgamma(const Image& image, const float g)
{
    Image tmp(image.width(), image.height());
    
    for(unsigned i= 0; i < image.size(); i++)
        tmp(i)= modgamma(image(i), g);
    
    return tmp;
}

// recup plage utile d'intensite
float range(const Image& image)
{
    ...
    {
        qbins += float(bins[i]) / float(image.size());
        if(qbins > .75f)
...
// export image avec gamma corrige
